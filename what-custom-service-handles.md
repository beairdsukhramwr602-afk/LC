...
Custom Service is the Next-Cart service model for migration projects that require customization, modification, exclusive handling, or bespoke configuration. It is used when the expected migration result cannot be achieved through the default migration tool capability or standard Add-ons alone.

Custom Service is broader than Add-ons. Add-ons support focused needs such as filtering, advanced mapping, or data configuration. Custom Service covers the wider set of requirements that need tailored handling, including Custom Cart, third-party data, custom fields, outside-system identifiers, migration-tool modification, and platform-specific transformation logic.

The purpose of Custom Service is to define a workable migration approach for requirements that do not fit a standard path.

### What Custom Service Means <a href="#what-custom-service-means" id="what-custom-service-means"></a>

Custom Service is required whenever the migration needs customization or modification work.
...
* interpreting a Custom Cart structure
...
> Any customization or modification work is handled through Custom Service.

This keeps the service path clear and prevents custom requirements from being treated as ordinary configuration.
...
Add-ons are focused features. They help customers handle specific migration needs such as data filtering, advanced mapping, or data configuration.

Custom Service is broader. It covers all customization and exclusive handling requirements, whether or not they involve an Add-on.
...
* handling Custom Cart, app-owned data, custom fields, or migration-tool behavior belongs under Custom Service

This distinction matters because not every custom migration need should be described as an Add-on.
...
### Custom Cart Handling <a href="#custom-cart-handling" id="custom-cart-handling"></a>

Custom Cart as Source Cart or Target Cart requires Custom Service.

A Custom Cart is a platform or store system that is not part of Next-Cart’s standard supported cart list. The challenge is not simply that the connection method may be different. The larger issue is that Custom Cart projects often require more interpretation before the data can be migrated safely.

Custom Cart handling may involve:
...
* transforming source data into a structure the Target Cart can support
...
Custom Cart migration should be planned carefully because the data structure may not follow a known supported-cart pattern.
...
The important question is not only whether the data exists. The important question is whether that data carries business meaning that must remain usable in the Target Cart.
...
Custom fields often require review because the Target Cart may not store or use the same type of field in the same way. The goal is to define the closest workable target result based on platform capability and business need.
...
Sometimes the Target Cart cannot represent source-store behavior exactly.
...
Custom Service helps define a migration approach when the expected result requires tailored handling within the Target Cart’s supported capabilities.
...
* reshaping content or metadata for the Target Cart
...
* whether Custom Cart is the Source Cart, Target Cart, or both
...
Custom Service provides a path for reviewing and implementing tailored handling. It does not mean every source-store behavior can always be recreated exactly in the Target Cart.

The final result still depends on:

* the quality and structure of the source data
* what the Target Cart can support
...
Custom Service handles the migration requirements that go beyond default migration-tool capability and standard Add-ons. It covers tailored Add-ons, custom Add-ons, Custom Cart handling, third-party data, custom fields, outside-system identifiers, migration-tool modification, bespoke transformation, and other exclusive handling needs.

The most important distinction is that Add-ons are focused features, while Custom Service is the broader path for customization and modification work. Custom Service also does not automatically mean Next-Cart performs the full migration process unless migration management is included in the final plan.

Prepare Custom Service requirements around the outcome that must be preserved, not only the field or data type that needs to move. If your project involves Custom Cart, third-party logic, custom fields, outside-system identifiers, tailored Add-ons, or custom Add-ons, Live Chat can help clarify what information Next-Cart needs to review the request and prepare a suitable quote.
...
#### Is Custom Service the same as Add-ons? <a href="#is-custom-service-the-same-as-add-ons" id="is-custom-service-the-same-as-add-ons"></a>
...
#### Is Custom Service required for modified Add-ons? <a href="#is-custom-service-required-for-modified-add-ons" id="is-custom-service-required-for-modified-add-ons"></a>

Yes. Any Add-on modification beyond default capability is handled through Custom Service.

#### Can Next-Cart provide custom Add-ons? <a href="#can-next-cart-provide-custom-add-ons" id="can-next-cart-provide-custom-add-ons"></a>
...
#### Is Custom Service required for Custom Cart? <a href="#is-custom-service-required-for-custom-cart" id="is-custom-service-required-for-custom-cart"></a>

Yes. Any migration involving Custom Cart as Source Cart or Target Cart requires Custom Service.

#### Does Custom Service always include migration management? <a href="#does-custom-service-always-include-migration-management" id="does-custom-service-always-include-migration-management"></a>
...
#### What information should I prepare for a Custom Service request? <a href="#what-information-should-i-prepare-for-a-custom-service-request" id="what-information-should-i-prepare-for-a-custom-service-request"></a>
